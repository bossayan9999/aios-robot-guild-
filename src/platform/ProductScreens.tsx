...
export function Integrations({ state }: { state: PlatformState }) {
  return <ScreenTitle eyebrow="VERIFIED CONNECTION REGISTRY" title="Integrations" detail="A connection is green only after the backend verifies credentials, permissions, provider identity, scopes, and live health."><div className="cs-connection-grid">{state.integrations.map(connection => <ConnectionCard connection={connection} key={connection.id} />)}</div><Panel title="Connector controls" eyebrow="SERVER-SIDE SECRETS ONLY"><div className="cs-assignment-list">{state.connectorRegistry?.connectors.filter(item => ['github', 'cloudflare', 'mcp'].includes(item.connector_id)).map(item => <div key={item.id}><span>{item.connector_id.slice(0, 2).toUpperCase()}</span><div><strong>{item.name}</strong><small>{item.state} · {item.provider_identity || 'provider identity unverified'}</small></div><div className="cs-actions">{['github', 'cloudflare'].includes(item.connector_id) && item.state === 'NOT_CONFIGURED' && <button disabled={state.busy} onClick={() => void state.configureConnector(item.connector_id)}>Use server secret reference</button>}{!['CONNECTED', 'REVOKED'].includes(item.state) && <button className="cs-button--primary" disabled={state.busy} onClick={() => void state.verifyConnector(item.connector_id)}>Verify live</button>}{item.state === 'CONNECTED' && <button disabled={state.busy} onClick={() => void state.revokeConnector(item.connector_id)}>Revoke</button>}</div></div>)}</div><p className="cs-callout">No secret value is accepted by this screen. GitHub and Cloudflare use named server-side secret references; missing or invalid credentials produce ERROR, never CONNECTED. Other providers remain Not configured until their verified adapter is implemented.</p></Panel></ScreenTitle>
}

export function RuntimeCenter({ state }: { state: PlatformState }) {
  return <ScreenTitle eyebrow="HYBRID EXECUTION" title="Runtime Center" detail="Every runtime reports a real, authenticated capability state."><div className="cs-connection-grid">{state.runtimes.map(connection => <ConnectionCard connection={connection} key={connection.id} />)}</div><Panel title="Execution leases" eyebrow="SERVER-AUTHORITATIVE"><div className="cs-assignment-list">{state.runtimeRegistry?.leases.length ? state.runtimeRegistry.leases.map(lease => <div key={lease.id}><span>{lease.action.slice(0, 2).toUpperCase()}</span><div><strong>{lease.action}</strong><small>Task {lease.task_id} · expires {new Date(lease.expires_at).toLocaleString()}</small></div><StatusPill state={lease.status === 'SUCCEEDED' ? 'passed' : lease.status === 'FAILED' || lease.status === 'CANCELLED' ? 'failed' : 'active'}>{lease.status}</StatusPill></div>) : <EmptyState title="No execution leases" detail="A lease appears only after an active specialist contract is bound to a fresh authenticated runtime." />}</div></Panel><Panel title="Runtime policy" eyebrow="SANDBOX FIRST"><div className="cs-policy-row"><div><strong>Deny by default</strong><small>Only task-scoped capabilities are granted.</small></div><div><strong>Short-lived leases</strong><small>Task, plan, roots, network policy and resource ceilings are immutable.</small></div><div><strong>Enrollment</strong><small>Pairing codes and credentials are returned once; only hashes are stored.</small></div></div></Panel></ScreenTitle>
}
...
export function AuditSecurity({ state }: { state: PlatformState }) {
  return <ScreenTitle eyebrow="CONTROL & ACCOUNTABILITY" title="Audit & Security" detail="Approvals, policy decisions, blocked work and emergency controls.">
    ...
    <div className="cs-grid cs-grid--thirds"><Panel title="Blocked operations" eyebrow="BACKEND TASK"><EmptyState title="No blocked-operation API" detail="A structured policy-decision endpoint is required." /></Panel><Panel title="Device revocation" eyebrow="RUNTIME REGISTRY"><p>{state.runtimeRegistry?.runtimes.filter(item => item.state === 'REVOKED').length || 0} runtime identities revoked. Revocation is available through the authenticated runtime API.</p></Panel><Panel title="Emergency stop" eyebrow="SAFETY CONTROL"><div className="cs-danger"><strong>Workspace execution stop</strong><p>Cancels active leases and suspends enrolled runtimes while preserving audit evidence.</p><button disabled={state.busy || !state.runtimeRegistry?.runtimes.length} onClick={() => void state.emergencyStop()}>Emergency stop</button></div></Panel></div>
  </ScreenTitle>
}

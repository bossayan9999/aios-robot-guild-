...
describe('Robot Guild product contract', () => {
  ...
  it('keeps deployment evidence and browser security headers', () => {
    ...
    expect(worker).toContain("const BUILD_ID = '2026.07.18-world1'")
    expect(app).toMatch(/const UI_BUILD = ["']2026\.07\.18-world1["']/)
  ...
  it('keeps the 3D Guild Hall interactive, mission-linked and mobile adjustable', () => {
    ...
    expect(hall).toContain('DRAG TO EXPLORE')
    expect(hall).toContain('Follow robot')
    expect(hall).toContain('state.current.progress')
    expect(hall).toContain('Wide original sandbox-game district')
    expect(hall).toContain('Active specialists walk from their workshop')
    expect(hall).toContain('new THREE.CapsuleGeometry(.58')

...
import type { AgentId, AuthStatus, DeploymentHealth, DeviceDiagnostics, KnowledgeHit, Mission, MissionEvent, ReleaseCenterStatus, ReleaseEvent, ReleaseProposal, SecurityLabStatus } from './types'
...
type Tab = 'guild' | 'quest' | 'missions' | 'studio' | 'devices' | 'network' | 'lab' | 'ops' | 'updates' | 'models' | 'knowledge' | 'settings'
...
const UI_BUILD = '2026.07.18-network3'
...
async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) { const requestId = response.headers.get('X-Request-ID'); throw new Error(`${body.error || body.detail || `Request failed (${response.status})`}${requestId ? ` • Request ${requestId}` : ''}`) }
  return body as T
}
...
export function App() {
  ...
  const [selectedRelease, setSelectedRelease] = useState<ReleaseProposal | null>(null)
  ...
  const [releaseGoal, setReleaseGoal] = useState('Improve the web app with the smallest safe, tested and reversible change.')
  ...
  useEffect(() => { if (auth?.authenticated && tab === 'settings') loadPasskeys() }, [tab, auth?.authenticated])
  useEffect(() => { if (auth?.authenticated && tab === 'updates') loadReleaseCenter() }, [tab, auth?.authenticated])

  const notify = (message: string) => { setToast(message); window.setTimeout(() => setToast(''), 2600) }
  async function checkDeployment() {
    setDeploymentChecking(true); setDeploymentError('')
    try {
      const response = await fetch(`/api/health?ui_build=${encodeURIComponent(UI_BUILD)}&t=${Date.now()}`, { cache: 'no-store' })
      const result = await response.json(); if (!response.ok) throw new Error(result.error || `Health check failed (${response.status})`)
      setDeploymentHealth({ ...result, request_id: response.headers.get('X-Request-ID') || undefined })
    } catch (error) { setDeploymentError(error instanceof Error ? error.message : 'Deployment check failed') } finally { setDeploymentChecking(false) }
  }
  ...
  async function loginWithPasskey() {
    setAuthError('')
    try {
      const options = await api<PublicKeyCredentialRequestOptionsJSON>('/api/passkeys/auth/options', { method: 'POST' })
      const response = await startAuthentication({ optionsJSON: options })
      await api('/api/passkeys/auth/verify', { method: 'POST', body: JSON.stringify({ challenge: options.challenge, response }) })
      setAuth(await api<AuthStatus>('/api/auth/status')); notify('Passkey accepted • vault unlocked')
    } catch (error) { setAuthError(error instanceof Error ? error.message : 'Passkey sign-in failed') }
  }
  async function loadPasskeys() {
    try { const result = await api<{ passkeys: PasskeySummary[] }>('/api/passkeys'); setPasskeys(result.passkeys) } catch { setPasskeys([]) }
  }
  async function addPasskey() {
    if (!window.PublicKeyCredential) return notify('Passkeys are not supported by this browser')
    try {
      const options = await api<PublicKeyCredentialCreationOptionsJSON>('/api/passkeys/register/options', { method: 'POST' })
      const response = await startRegistration({ optionsJSON: options })
      await api('/api/passkeys/register/verify', { method: 'POST', body: JSON.stringify({ challenge: options.challenge, name: 'Primary device', response }) })
      await loadPasskeys(); notify('Passkey added securely')
    } catch (error) { notify(error instanceof Error ? error.message : 'Passkey registration failed') }
  }
  async function deletePasskey(id: string) {
    if (!window.confirm('Remove this passkey from AIOS Robot Guild?')) return
    try { await api(`/api/passkeys/${encodeURIComponent(id)}`, { method: 'DELETE' }); await loadPasskeys(); notify('Passkey removed') } catch (error) { notify(error instanceof Error ? error.message : 'Passkey removal failed') }
  }
  async function loadReleaseCenter() {
    try {
      const [status, list] = await Promise.all([api<ReleaseCenterStatus>('/api/releases/status'), api<{ releases: ReleaseProposal[] }>('/api/releases')])
      setReleaseStatus(status); setReleases(list.releases)
    } catch (error) { notify(error instanceof Error ? error.message : 'Update Center unavailable') }
  }
  async function selectRelease(item: ReleaseProposal) {
    setSelectedRelease(item)
    try { const result = await api<{ proposal: ReleaseProposal; events: ReleaseEvent[] }>(`/api/releases/${item.id}`); setSelectedRelease(result.proposal); setReleaseEvents(result.events) } catch (error) { notify(error instanceof Error ? error.message : 'Release evidence unavailable') }
  }
  async function createReleaseProposal(event: React.FormEvent) {
    event.preventDefault()
    try {
      const result = await api<{ proposal: ReleaseProposal; events: ReleaseEvent[] }>('/api/releases', { method: 'POST', body: JSON.stringify({ title: releaseTitle, goal: releaseGoal }) })
      setSelectedRelease(result.proposal); setReleaseEvents(result.events); await loadReleaseCenter(); notify('Release plan created • approval required')
    } catch (error) { notify(error instanceof Error ? error.message : 'Release proposal failed') }
  }
  async function decideRelease(decision: 'approved' | 'rejected') {
    if (!selectedRelease || !window.confirm(`${decision === 'approved' ? 'Approve' : 'Reject'} this release plan?\n\nNo code will be changed by this approval.`)) return
    try {
      const result = await api<{ proposal: ReleaseProposal; message: string }>(`/api/releases/${selectedRelease.id}/approval`, { method: 'POST', body: JSON.stringify({ decision }) })
      setSelectedRelease(result.proposal); await selectRelease(result.proposal); await loadReleaseCenter(); notify(result.message)
    } catch (error) { notify(error instanceof Error ? error.message : 'Release decision failed') }
  }
  ...
  async function changePassword(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (demoMode) return notify('Password changes are unavailable in demo mode')
    ...
    if (password !== confirmation) return notify('The new passwords do not match')
    try {
      await api('/api/auth/password', { method: 'POST', body: JSON.stringify({ password }) })
      form.reset()
      notify('Password changed • other sessions signed out')
    } catch (error) { notify(error instanceof Error ? error.message : 'Password change failed') }
  }
  ...
  return <div className="app-shell">
    {!unlocked && <div className="auth-overlay"><form className="auth-card" onSubmit={authenticate}><div className="crest">A</div><p className="kicker">PRIVATE GUILD ACCESS</p><h1>{authMode === 'setup' ? 'Create the owner vault' : 'Enter Robot Guild'}</h1><p>{authMode === 'setup' ? 'Claim the first secure owner account.' : 'Sign in to continue your missions.'}</p><label>Email<input name="email" type="email" required autoComplete="username webauthn" /></label><label>Password<input name="password" type="password" minLength={10} required autoComplete={authMode === 'setup' ? 'new-password' : 'current-password'} /></label><button className="primary" type="submit">{authMode === 'setup' ? 'Create secure vault' : 'Sign in'}</button>{authMode === 'login' && <button type="button" className="passkey-button" onClick={loginWithPasskey}>Sign in with a passkey</button>}{authError && <p className="error">{authError}</p>}<div className="auth-divider"><span>or</span></div><button type="button" onClick={() => { setDemoMode(true); notify('Demo mode: data stays in this browser') }}>Explore demo mode</button><small>API keys are never entered in this browser.</small></form></div>}

    <aside className="sidebar"><div className="brand"><div className="crest">A</div><div><b>AIOS</b><span>Robot Guild</span></div></div><Nav label="Robot Guild" icon="⚔" active={tab === 'guild'} onClick={() => setTab('guild')} /><Nav label="Health Quest" icon="🛡" active={tab === 'quest'} onClick={() => setTab('quest')} /><Nav label="Mission History" icon="📜" active={tab === 'missions'} onClick={() => setTab('missions')} /><Nav label="Developer Studio" icon="⌘" active={tab === 'studio'} onClick={() => setTab('studio')} /><Nav label="Devices" icon="▣" active={tab === 'devices'} onClick={() => setTab('devices')} /><Nav label="CCNA Network" icon="⌁" active={tab === 'network'} onClick={() => setTab('network')} /><Nav label="Security Lab" icon="⬡" active={tab === 'lab'} onClick={() => setTab('lab')} /><p className="nav-title">SYSTEM</p><Nav label="Deployment" icon="◌" active={tab === 'ops'} onClick={() => setTab('ops')} /><Nav label="Update Center" icon="↻" active={tab === 'updates'} onClick={() => setTab('updates')} /><Nav label="AI Models" icon="✦" active={tab === 'models'} onClick={() => setTab('models')} /><Nav label="Knowledge" icon="◈" active={tab === 'knowledge'} onClick={() => setTab('knowledge')} /><Nav label="Settings" icon="⚙" active={tab === 'settings'} onClick={() => setTab('settings')} /><div className="profile"><div className="level">{level}</div><div><b>Guild Owner</b><small>Level {level} • {xp} XP</small></div></div></aside>

    <main><header><div><p className="kicker">AIOS SOFTWARE FACTORY • RECONSTRUCTED SOURCE</p><h1>{tab === 'guild' ? 'Your agent guild, alive.' : tab === 'quest' ? 'Repository Health Quest' : tab === 'studio' ? 'Developer Studio' : tab === 'devices' ? 'Device Command Center' : tab === 'network' ? 'CCNA Network Guild' : tab === 'lab' ? 'Authorized Security Lab' : tab === 'ops' ? 'Deployment Control Center' : tab === 'updates' ? 'Update Center' : tab === 'models' ? 'AI Model Hub' : tab === 'missions' ? 'Mission Archive' : tab === 'knowledge' ? 'Verified Knowledge' : 'Guild Settings'}</h1></div><div className="header-actions"><span className="online">● {demoMode ? 'DEMO' : 'BACKEND'} ONLINE</span><button onClick={logout}>Log out</button></div></header>
      ...
      {tab === 'updates' && <section className="update-layout"><article className="panel-card"><div className="section-head"><div><p className="kicker">OWNER-GATED RELEASE CONTROL</p><h2>Turn a goal into a safe release</h2></div><button onClick={loadReleaseCenter}>Refresh</button></div><div className="connection-strip"><span className={releaseStatus?.github_connected ? 'connected' : 'waiting'}>GitHub App • {releaseStatus?.github_connected ? 'ready' : 'not connected'}</span><span className={releaseStatus?.cloudflare_connected ? 'connected' : 'waiting'}>Cloudflare • {releaseStatus?.cloudflare_connected ? 'online' : 'unavailable'}</span><span>Build • {releaseStatus?.build || UI_BUILD}</span></div><form className="release-form" onSubmit={createReleaseProposal}><label>Release title<input value={releaseTitle} onChange={event => setReleaseTitle(event.target.value)} minLength={3} maxLength={120} required /></label><label>Improvement goal<textarea value={releaseGoal} onChange={event => setReleaseGoal(event.target.value)} minLength={10} maxLength={2000} required /></label><button className="primary" type="submit">Create release plan</button></form><div className="release-pipeline">{['Goal', 'Plan', 'Branch / PR', 'Tests', 'Owner approval', 'Cloudflare'].map((stage, index) => <div key={stage} className={index < 2 ? 'ready' : ''}><i>{index + 1}</i><span>{stage}</span></div>)}</div><div className="notice">Version 1 stores proposals, decisions and evidence. It never edits GitHub or deploys production. Branch/PR execution unlocks only after a repository-scoped GitHub App is configured and tested.</div></article><aside><article className="panel-card"><p className="kicker">RELEASE QUEUE</p><h2>Durable proposals</h2><div className="release-list">{releases.length ? releases.map(item => <button key={item.id} className={selectedRelease?.id === item.id ? 'active' : ''} onClick={() => selectRelease(item)}><span><b>{item.title}</b><small>{item.id} • {new Date(item.created_at).toLocaleString()}</small></span><em>{item.status.replaceAll('_', ' ')}</em></button>) : <p className="muted">No release proposals yet.</p>}</div></article>{selectedRelease && <article className="panel-card release-evidence"><p className="kicker">RELEASE {selectedRelease.id}</p><h2>{selectedRelease.title}</h2><p>{selectedRelease.goal}</p><pre>{selectedRelease.plan}</pre><div className="release-events">{releaseEvents.map(item => <div key={item.id}><b>{item.stage} • {item.event_type.replaceAll('_', ' ')}</b><p>{item.message}</p></div>)}</div>{selectedRelease.status === 'planned' && <div className="actions"><button onClick={() => decideRelease('rejected')}>Reject</button><button className="primary" onClick={() => decideRelease('approved')}>Approve plan</button></div>}{selectedRelease.status === 'approved_waiting_connection' && <div className="notice">Approved and paused safely. Connect a repository-scoped GitHub App before enabling branch or pull-request creation.</div>}</article>}</aside></section>}
      ...
      {tab === 'settings' && <section className="settings-grid"><article className="panel-card"><p className="kicker">APPEARANCE</p><h2>Theme</h2><div className="theme-options"><button onClick={() => setTheme('mission')}>Mission dark</button><button onClick={() => setTheme('warm')}>Warm paper</button><button onClick={() => setTheme('slate')}>Slate blue</button></div></article><article className="panel-card"><p className="kicker">OWNER SECURITY</p><h2>Change password</h2><p>Set a new password from this trusted signed-in device. Every other session will be signed out.</p><form className="password-form" onSubmit={changePassword}><label>New password<input name="newPassword" type="password" minLength={12} maxLength={128} required autoComplete="new-password" placeholder="12 characters or more" /></label><label>Confirm new password<input name="confirmPassword" type="password" minLength={12} maxLength={128} required autoComplete="new-password" /></label><button className="primary" type="submit" disabled={demoMode}>Change password and rotate sessions</button></form><small className="security-note">The password is sent only to the authenticated Worker and stored as a salted hash.</small></article><article className="panel-card"><p className="kicker">PASSKEYS</p><h2>Passwordless sign-in</h2><p>Use your phone, fingerprint, face, Windows Hello, or security key. Passkeys are bound to this HTTPS domain.</p><button className="primary" onClick={addPasskey}>Add a passkey</button><div className="passkey-list">{passkeys.length ? passkeys.map(item => <div key={item.id}><span><b>{item.name}</b><small>{item.device_type.replaceAll('_', ' ')} • {item.backed_up ? 'synced backup' : 'device-bound'}</small></span><button type="button" onClick={() => deletePasskey(item.id)}>Remove</button></div>) : <p className="muted">No passkeys registered yet. Keep your password as a recovery method.</p>}</div></article><article className="panel-card"><p className="kicker">AI CONNECTIONS</p><h2>Models and tools</h2><p>Search providers, open official key pages, then store credentials only as Cloudflare secrets.</p><div className="tool-actions"><button onClick={() => setTab('models')}>Search AI models</button><button onClick={() => setTab('studio')}>Open Developer Studio</button></div></article><article className="panel-card"><p className="kicker">SECURITY</p><h2>Backend rules</h2><ul><li>D1-backed owner login and passkeys</li><li>HTTP-only session cookie</li><li>Server-side provider secrets</li><li>Explicit human approval</li><li>Read-only GitHub inspection</li></ul></article><article className="panel-card"><p className="kicker">RECOMMENDED NEXT TOOLS</p><h2>Backend capability stack</h2><ul><li>Cloudflare Queues for durable agent jobs</li><li>Vectorize for cited knowledge retrieval</li><li>GitHub App with repository-scoped permissions</li><li>Sentry or Workers Logs for errors</li><li>Stripe only when SaaS billing is ready</li></ul></article></section>}
    ...
    {tab === 'network' && <CCNANetworkLab onEarnXp={amount => { setXp(value => value + amount); notify(`+${amount} Network XP • quest complete`) }} />}
    ...
    {tab === 'ops' && <DeploymentCenter health={deploymentHealth} error={deploymentError} checking={deploymentChecking} uiBuild={UI_BUILD} onRefresh={checkDeployment} />}
  </div>
}
...
function DeploymentCenter({ health, error, checking, uiBuild, onRefresh }: { health: DeploymentHealth | null; error: string; checking: boolean; uiBuild: string; onRefresh: () => void }) {
  const matched = Boolean(health && health.build === uiBuild)
  return <section className="ops-overlay"><div className="ops-grid"><article className="panel-card"><div className="section-head"><div><p className="kicker">LIVE RELEASE VERIFICATION</p><h2>{error ? 'Health check failed' : matched ? 'Release synchronized' : health ? 'Deployment mismatch' : 'Checking deployment'}</h2></div><span className={`ops-badge ${error ? 'fail' : matched ? 'pass' : 'warn'}`}>{error ? 'ERROR' : matched ? 'PASS' : 'REVIEW'}</span></div><p>The UI and Worker must report the same build identifier. A mismatch means GitHub or Cloudflare is serving an older release.</p><div className="ops-facts"><div><small>UI build</small><code>{uiBuild}</code></div><div><small>Worker build</small><code>{health?.build || 'Unavailable'}</code></div><div><small>API version</small><b>{health?.version || 'Unavailable'}</b></div><div><small>Checked</small><b>{health?.checked_at ? new Date(health.checked_at).toLocaleString() : 'Not yet'}</b></div></div>{error && <div className="notice ops-error">{error}</div>}{health && !matched && <div className="notice ops-warning">Cloudflare is serving a different backend build. Redeploy the latest GitHub main commit, then refresh this check.</div>}<div className="device-actions"><button className="primary" disabled={checking} onClick={onRefresh}>{checking ? 'Checking…' : 'Run live check'}</button><a className="button-link" href="https://dash.cloudflare.com/" target="_blank" rel="noreferrer">Open Cloudflare ↗</a><a className="button-link" href="https://github.com/bossayan9999/aios-robot-guild-/commits/main" target="_blank" rel="noreferrer">View GitHub commits ↗</a></div></article><aside><article className="panel-card"><p className="kicker">SERVICE CHECKS</p><div className="doctor-checks"><div className={`doctor-check ${health?.ok ? 'pass' : 'fail'}`}><i>{health?.ok ? '✓' : '×'}</i><div><b>Worker API</b><small>{health?.ok ? 'Health endpoint responded' : 'No healthy response'}</small></div></div><div className={`doctor-check ${health?.checks?.assets === 'pass' ? 'pass' : 'warn'}`}><i>{health?.checks?.assets === 'pass' ? '✓' : '!'}</i><div><b>Static assets</b><small>{health?.checks?.assets || 'Unknown'}</small></div></div><div className={`doctor-check ${matched ? 'pass' : 'warn'}`}><i>{matched ? '✓' : '!'}</i><div><b>Version contract</b><small>{matched ? 'UI and Worker match' : 'Redeployment may be required'}</small></div></div></div></article><article className="panel-card"><p className="kicker">SUPPORT EVIDENCE</p><h2>Request ID</h2><code className="ops-request">{health?.request_id || 'Generated after a successful check'}</code><p className="muted">Copy this ID when reporting an error so logs can be correlated without exposing secrets.</p></article><article className="panel-card"><p className="kicker">RELEASE CHECKLIST</p><ol><li>Tests and build pass</li><li>GitHub main contains the release</li><li>Cloudflare deployment is active</li><li>UI and Worker build IDs match</li><li>Authenticated smoke test passes</li><li>Rollback version is recorded</li></ol></article></aside></div></section>
}

...
describe('Robot Guild product contract', () => {
  ...
  it('keeps Forge communication interactive and browser-permission gated', () => {
    ...
    expect(worker).toContain("SELECT COUNT(*) AS count FROM missions WHERE user_id=? AND status='completed'")
  ...
  it('grows the engineering and OSINT guild only after completed missions', () => {
    ...
    expect(worker).toContain("m.status='completed'")
    ...
    expect(worker).toContain("match[2] === 'verification'")
    expect(worker).toContain("input.decision === 'completed'")
    expect(app).toContain('Verify completion')
    expect(app).toContain('Rewards remain locked until you confirm the mission is complete')
    expect(hall).toContain("missionStatus === 'completed'")

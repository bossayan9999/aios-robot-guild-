...
type Tab = 'guild' | 'quest' | 'missions' | 'studio' | 'models' | 'knowledge' | 'settings'
...
export function App() {
  ...
  const [copilotMessages, setCopilotMessages] = useState<{ role: 'user' | 'copilot'; text: string }[]>([{ role: 'copilot', text: 'I am Forge, your developer Copilot. I can explain missions, models, safe terminal checks, and the next build step.' }])
  ...
  async function askCopilot(event: React.FormEvent) {
    ...
    try { const result = await api<{ answer: string }>('/api/copilot', { method: 'POST', body: JSON.stringify({ question }) }); setCopilotMessages(current => [...current, { role: 'copilot', text: result.answer }]) } catch (error) { setCopilotMessages(current => [...current, { role: 'copilot', text: error instanceof Error ? error.message : 'Copilot unavailable.' }]) }
  }
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><div className="brand"><div className="crest">A</div><div><b>AIOS</b><span>Robot Guild</span></div></div><Nav label="Robot Guild" icon="⚔" active={tab === 'guild'} onClick={() => setTab('guild')} /><Nav label="Health Quest" icon="🛡" active={tab === 'quest'} onClick={() => setTab('quest')} /><Nav label="Mission History" icon="📜" active={tab === 'missions'} onClick={() => setTab('missions')} /><Nav label="Developer Studio" icon="⌘" active={tab === 'studio'} onClick={() => setTab('studio')} /><p className="nav-title">SYSTEM</p><Nav label="AI Models" icon="✦" active={tab === 'models'} onClick={() => setTab('models')} /><Nav label="Knowledge" icon="◈" active={tab === 'knowledge'} onClick={() => setTab('knowledge')} /><Nav label="Settings" icon="⚙" active={tab === 'settings'} onClick={() => setTab('settings')} /><div className="profile"><div className="level">{level}</div><div><b>Guild Owner</b><small>Level {level} • {xp} XP</small></div></div></aside>

    <main><header><div><p className="kicker">AIOS SOFTWARE FACTORY • RECONSTRUCTED SOURCE</p><h1>{tab === 'guild' ? 'Your agent guild, alive.' : tab === 'quest' ? 'Repository Health Quest' : tab === 'studio' ? 'Developer Studio' : tab === 'models' ? 'AI Model Hub' : tab === 'missions' ? 'Mission Archive' : tab === 'knowledge' ? 'Verified Knowledge' : 'Guild Settings'}</h1></div><div className="header-actions"><span className="online">● {demoMode ? 'DEMO' : 'BACKEND'} ONLINE</span><button onClick={logout}>Log out</button></div></header>
